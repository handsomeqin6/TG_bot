"""
monitor.py — On-chain payment verification via transaction history only.

Core principle:
  - check_payment() is called exclusively from _poll_payments() in main.py.
  - _poll_payments() only iterates addresses that have a pending DB record
    (invite_sent=0).  If the DB is empty there is nothing to iterate and no
    blockchain queries are made.
  - Payment is confirmed solely by finding a qualifying on-chain transaction.
    Balance checks are never used to confirm payment.
"""
import logging
import time
...
_TRON_BLOCK_INTERVAL_MS = 3_000   # ~1 block per 3 s on TRON
...
async def _trc20_transfers(session: aiohttp.ClientSession, address: str) -> list:
    """Return standard TRC20 Transfer event records for address."""
...
async def _general_txs_for_usdt(session: aiohttp.ClientSession, address: str) -> list:
    """
    Fallback: query all transaction types and extract USDT transfers by ABI parsing.
    Catches GasFree and other non-standard paths missed by the TRC20 endpoint.
    Returns records in the same shape as _trc20_transfers output.
    """
    ...
    for tx in data.get("data", []):
        ...
        # transfer(address,uint256): selector a9059cbb + 32-byte addr + 32-byte amount
        if len(hex_data) < 136 or hex_data[:8] != "a9059cbb":
            continue

        recipient_evm = hex_data[32:72]   # last 20 bytes of the 32-byte address param
...
async def _tx_block_number(session: aiohttp.ClientSession, tx_id: str) -> int:
    """Return the block number a transaction was included in (0 on failure)."""
    async with session.post(
        ...
        if r.status != 200:
            return 0
        return int((await r.json(content_type=None)).get("blockNumber", 0))


def _confirmed_by_timestamp(block_timestamp_ms: int) -> bool:
    """Fast-path: estimate confirmations from block_timestamp (no extra API call)."""
...
async def _check_tx_list(
    ...
    """
    Walk a list of transactions and return the first one that is both
    sufficiently large and sufficiently confirmed.
    """
    for tx in txs:
...
async def check_payment(address: str) -> Optional[dict]:
    """
    Return {"tx_id": str, "amount_sun": int} when a confirmed USDT payment is found,
    or None if no qualifying transaction exists yet.

    This function ONLY inspects on-chain transaction history — never account balances.
    It must only be called for addresses that have an active pending order in the DB
    (enforced by _poll_payments in main.py).

    Query order:
      1. /transactions/trc20  — standard TRC20 Transfer events
      2. /transactions        — all tx types, ABI-parsed (GasFree / non-standard fallback)
    """
    try:
        async with aiohttp.ClientSession(headers=_HEADERS) as session:
            ...
            trc20_txs = [t for t in trc20_txs
                         if t.get("to") == address
